...
- **SSR** : 처음 클라이언트가 브라우저에 접속했을때, 브라우저가 JS파일을 모두 다운받을 때 까지 기다리지 않고, 서버에서 랜더링을 마치고 데이터가 결합된 HTML파일을 보내준다.
  - 장점
    - CSR에 비해 첫페이지 로딩속도가 빠르다.
    - SEO가 좋다.
  - 단점
    - 새로운 페이지로 이동할 때 마다 서버에서 랜더링된 페이지를 받아야하므로 페이지 깜빡임이 발생할 수 있다.
    - TTV와 TTI사이의 시간차이가 발생하여 응답하지 않는 페이지를 마주할 수 있다.



- **CSR** : 최초 요청시에 HTML을 비롯해 CSS, JS등 각종 리소스 파일을 모두 받아오게 된다. 이후에 서베에는 데이터만 요청하게 되고, 모든 랜더링 작업은 클라이언트에서 이루어지게 된다.
  - 장점
    - SSR보다 빠른 페이지 전환속도, 더 나은 사용자경험 제공
  - 단점
    - 첫 페이지 로딩속도가 느리다.
    - 빈 HTML파일이므로 SEO가 좋지 못하다.
